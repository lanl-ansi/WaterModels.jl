@testset "Optimal Water Flow Problems (Single Network)" begin
    ...
    wm = instantiate_model(network, NLPWaterModel, build_owf_agm)
    ...
    wm = instantiate_model(network, MICPRWaterModel, build_owf_agm, ext=Dict(:pump_breakpoints=>3))
    result = WaterModels.optimize_model!(wm, optimizer=_make_juniper(wm, ipopt))
    @test result["termination_status"] == LOCALLY_SOLVED
    @test isapprox(result["solution"]["node"]["1"]["h"], 10.0, rtol=1.0e-3)
    ...
    result = run_owf_agm(network, MILPWaterModel, cbc, ext=Dict(:pump_breakpoints=>4))
...
@testset "Optimal Water Flow Problems (Multinetwork)" begin
    ...
    wm = instantiate_model(network, NLPWaterModel, build_mn_owf_agm)
    result = WaterModels.optimize_model!(wm, optimizer=_make_juniper(wm, ipopt))
    @test result["termination_status"] == LOCALLY_SOLVED

    wm = instantiate_model(network, MICPRWaterModel, build_mn_owf_agm, ext=Dict(:pump_breakpoints=>3))
    result = WaterModels.optimize_model!(wm, optimizer=_make_juniper(wm, ipopt))
    @test result["termination_status"] == LOCALLY_SOLVED

    result = run_mn_owf_agm(network, MILPWaterModel, cbc, ext=Dict(:pump_breakpoints=>4))
    @test result["termination_status"] == OPTIMAL

    result = run_mn_owf_agm(network, MILPRWaterModel, cbc, ext=Dict(:pump_breakpoints=>3))
    @test result["termination_status"] == OPTIMAL
end
