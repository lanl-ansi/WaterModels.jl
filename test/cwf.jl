@testset "Constrained Water Flow Problems" begin
    ...
    @testset "Example 1 network (with tanks and pumps), multinetwork CNLP formulation." begin
        ...
        # The CNLP formulation does not support tanks, so an error is thrown.
        @test_throws ErrorException build_model(mn_data, CNLPWaterModel, WaterModels.post_mn_cwf, multinetwork=true)
    end
    ...
    @testset "Shamir network, multinetwork CNLP formulation." begin
        ...
        mn_data = WaterModels.make_multinetwork(data)
        wm = build_model(mn_data, CNLPWaterModel, WaterModels.post_mn_cwf, multinetwork=true)
    ...
    #    # TODO: The below takes to long to execute.
    ...
    #    # The MICP formulation does not support tanks, so an error is thrown.
    #    @test_throws ErrorException run_cwf("../test/data/epanet/example_1-sp.inp", MICPWaterModel, ipopt)
    #end
    ...
    #    # The MICP formulation does not support tanks, so an error is thrown.
    #    @test_throws ErrorException build_model(mn_data, MICPWaterModel, WaterModels.post_mn_cwf, multinetwork=true)
    #end

    #@testset "Richmond (single time) network, MICP formulation." begin
    #    # The MICP formulation does not support tanks, so an error is thrown.
    #    @test_throws ErrorException run_cwf("../test/data/epanet/richmond-skeleton-sp.inp", MICPWaterModel, ipopt)
    #end
    ...
    #    modifications = WaterModels.parse_file("../test/data/json/balerma.json")
    #    InfrastructureModels.update_data!(data, modifications)
    #    wm = build_model(data, MILPWaterModel, WaterModels.post_cwf, ext=Dict(:num_breakpoints => 25))
    ...
    #@testset "Example 1 network (with tanks and pumps), MILP formulation." begin
    #    data = WaterModels.parse_file("../test/data/epanet/example_1-sp.inp")
    #    wm = build_model(data, MILPWaterModel, WaterModels.post_cwf)
    #    f = Juniper.register(fun(wm, :head_loss)..., autodiff=false)
    #    juniper = JuMP.with_optimizer(Juniper.Optimizer, nl_solver=ipopt, registered_functions=[f], log_levels=[])
    #    solution = WaterModels.optimize_model!(wm, juniper)
    ...
    #@testset "Example 1 network (with tanks and pumps), multinetwork MILP formulation." begin
    #    data = WaterModels.parse_file("../test/data/epanet/example_1.inp")
    #    mn_data = WaterModels.make_multinetwork(data)
    #    wm = build_model(mn_data, MILPWaterModel, WaterModels.post_mn_cwf, multinetwork=true)
    #    f = Juniper.register(fun(wm, :head_loss)..., autodiff=false)
    #    juniper = JuMP.with_optimizer(Juniper.Optimizer, nl_solver=ipopt, registered_functions=[f], log_levels=[])
    #    solution = WaterModels.optimize_model!(wm, juniper)
    ...
    #    solution = run_cwf("../test/data/epanet/shamir.inp", MILPWaterModel, ipopt)
    #    @test solution["termination_status"] == LOCALLY_SOLVED
    #    @test isapprox(solution["solution"]["pipe"]["2"]["q"], 0.093565, rtol=1.0e-4)
    #    @test isapprox(solution["solution"]["pipe"]["6"]["q"], 0.055710, rtol=1.0e-4)
    #    @test isapprox(solution["solution"]["node"]["2"]["h"], 203.247650, rtol=1.0e-4)
