    ...
    @testset "Water Flow Problems (Single Network): $(formulation)" begin
        ...
        for (component, name) in Dict("Pipe" => "pipe", "Check Valve" => "check_valve", "Shutoff Valve" => "shutoff_valve")
            @testset "Hazen-Williams Head Loss ($(component)): $(formulation)" begin
                ...
                result = WaterModels.optimize_model!(wm, optimizer = _choose_solver(wm, ipopt, highs))
...
@testset "solve_wf" begin
    ...
    result = WaterModels.solve_wf(network, LRDWaterModel, highs)
    result = WaterModels.run_wf(network, LRDWaterModel, highs)
    @test _is_valid_status(result["termination_status"])
end
...
@testset "solve_mn_wf" begin
    ...
    result = WaterModels.solve_mn_wf(network_mn, LRDWaterModel, highs)
    result = WaterModels.run_mn_wf(network_mn, LRDWaterModel, highs)
    @test _is_valid_status(result["termination_status"])
end
...
@testset "solve_mn_wf_switching" begin
    ...
    result = WaterModels.solve_mn_wf_switching(network_mn, LRDWaterModel, highs)
    result = WaterModels.run_mn_wf_switching(network_mn, LRDWaterModel, highs)
    @test _is_valid_status(result["termination_status"])
end
