...
function constraint_on_off_pressure_drop{T}(wm::GenericWaterModel{T}, pipe)
    pipe_idx = pipe["index"]
    ...
    pi = getvariable(wm.model, :p_gas)[i_junction_idx]
    pj = getvariable(wm.model, :p_gas)[j_junction_idx]
    
    pd_min = pipe["pd_min"]
    pd_max = pipe["pd_max"]
      
    c1 = @constraint(wm.model, (1-yp) * pd_min <= pi - pj)
    c2 = @constraint(wm.model, pi - pj <= (1-yn)* pd_max)
            
    return Set([c1,c2])  # checked (though one is reversed of the other)
end

# constraints on pressure drop across pipes
function constraint_on_off_pressure_drop_fixed_direction{T}(wm::GenericWaterModel{T}, pipe)
    pipe_idx = pipe["index"]
    i_junction_idx = pipe["f_junction"]
    j_junction_idx = pipe["t_junction"]
...
function constraint_on_off_pipe_flow_direction{T}(wm::GenericWaterModel{T}, pipe)
    pipe_idx = pipe["index"]
    i_junction_idx = pipe["f_junction"]
    j_junction_idx = pipe["t_junction"]
...
function constraint_on_off_pipe_flow_direction_fixed_direction{T}(wm::GenericWaterModel{T}, pipe)
    pipe_idx = pipe["index"]
    i_junction_idx = pipe["f_junction"]
    j_junction_idx = pipe["t_junction"]
...
function constraint_on_off_compressor_flow_direction{T}(wm::GenericWaterModel{T}, compressor)
    c_idx = compressor["index"]
    i_junction_idx = compressor["f_junction"]
    j_junction_idx = compressor["t_junction"]

    yp = getvariable(wm.model, :yp)[c_idx]
    yn = getvariable(wm.model, :yn)[c_idx]
    f = getvariable(wm.model, :f)[c_idx]

    c1 = @constraint(wm.model, -(1-yp)*wm.data["max_flow"] <= f)
    c2 = @constraint(wm.model, f <= (1-yn)*wm.data["max_flow"])
      
    return Set([c1, c2])      # checked (though 1 is reversed)
end 

# constraints on flow across compressors when directions are constants
function constraint_on_off_compressor_flow_direction_fixed_direction{T}(wm::GenericWaterModel{T}, compressor)
    c_idx = compressor["index"]
    i_junction_idx = compressor["f_junction"]
    j_junction_idx = compressor["t_junction"]
...
# constraints on flow across compressors
function constraint_on_off_compressor_flow_direction_ne{T}(wm::GenericWaterModel{T}, compressor)
    c_idx = compressor["index"]
    i_junction_idx = compressor["f_junction"]
    j_junction_idx = compressor["t_junction"]

    yp = getvariable(wm.model, :yp_ne)[c_idx]
    yn = getvariable(wm.model, :yn_ne)[c_idx]
    f = getvariable(wm.model, :f_ne)[c_idx]
...
function constraint_on_off_compressor_flow_direction_ne_fixed_direction{T}(wm::GenericWaterModel{T}, compressor)
    c_idx = compressor["index"]
    i_junction_idx = compressor["f_junction"]
    j_junction_idx = compressor["t_junction"]
...
function constraint_on_off_compressor_ratios{T}(wm::GenericWaterModel{T}, compressor)
    c_idx = compressor["index"]
    i_junction_idx = compressor["f_junction"]
    j_junction_idx = compressor["t_junction"]
      
    i = wm.set.junctions[i_junction_idx]  
    j = wm.set.junctions[j_junction_idx]  
