...
function constraint_directed_flow_conservation(wm::GenericWaterModel, i::Int, n::Int=wm.cnw) #where T <: StandardMILPRForm
    ...
    for (a, conn) in filter(a -> i == parse(Int, a.second["node2"]), connections)
        flow_sum += sum(wm.var[:nw][n][:qp][a])
        flow_sum -= sum(wm.var[:nw][n][:qn][a])
    end
