...
# @NLconstraint(model, temperature_loss_steam[e in SetPO], T_out[e] <=  T_in[e])
...
# @NLconstraint(model, temperature_loss_water[e in SetPR], T_out[e] <= T_in[e])
#
# # # #Pressure loss steam
...
# @NLconstraint(model, pressure_loss_steam[e in SetPO], p[N_to[e]] == p[N_fr[e]])
# # # #Pressure loss water
@NLconstraint(model, pressure_loss_water[e in SetPR], p[N_fr[e]] - p[N_to[e]] == phi[e]*abs(phi[e])*lambda[e]*L[e]/(2*d[e]*rho[e]))
# @NLconstraint(model, pressure_loss_water[e in SetPR], p[N_fr[e]] >= p[N_to[e]])
#
# # # #Plant
...
@constraint(model, load_pressure_with_pipe_after_condensor_1[e in LoadSet], p[N_fr[e]] >= p[N_to[e]] - p_tol)
# @constraint(model, load_pressure_with_pipe_after_condensor_2[e in LoadSet], p[N_fr[e]] <= p[N_to[e]] + p_tol)
#
...
# Minimize energy: so change plant outlet temperature and see how the energy varies and how the flow rate varies
