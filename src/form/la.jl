...
function constraint_on_off_des_pipe_head_loss(
    ...
    # Get the corresponding set of flow breakpoints.
    @assert haskey(ref(wm, n, :des_pipe, a), "flow_breakpoints")
    breakpoints = ref(wm, n, :des_pipe, a, "flow_breakpoints")
    bp_range, bp_range_m1 = 1:length(breakpoints), 1:length(breakpoints)-1
...
function constraint_on_off_pump_power(
    ...
    power_expr = sum(f_all[k] * lambda[a, k] for k = 1:length(breakpoints))
