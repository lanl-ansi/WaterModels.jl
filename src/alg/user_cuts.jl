...
function solution_is_integer(wm::GenericWaterModel, lp_solution::Array{Float64,1}, n::Int = wm.cnw)
    ...
    dir_zeros = filter(x -> isapprox(x, 0.0, atol = 1.0e-6), dir_sol)
    dir_ones = filter(x -> isapprox(x, 1.0, atol = 1.0e-6), dir_sol)
    ...
    for (a, connection) in wm.ref[:nw][n][:connection]
        ...
        xr_zeros = filter(x -> isapprox(x, 0.0, atol = 1.0e-4), xr_sol)
        xr_ones = filter(x -> isapprox(x, 1.0, atol = 1.0e-4), xr_sol)
