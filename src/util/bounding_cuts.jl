...
function add_upper_approximation(wm::GenericWaterModel,
    ...
    for (a, connection) in wm.ref[:nw][n][:connection]
        ...
        r_to_bound = intersect(1:length(R_a), [r_hat, r_hat - 1, r_hat + 1])
        ...
        if q[a] > 0.0
            ...
            q_1 = getlowerbound(wm.var[:nw][n][:qp][a][r_hat])
            q_4 = getupperbound(wm.var[:nw][n][:qp][a][r_hat])
            q_2 = max(q_1, q[a] - 0.01)
            q_3 = min(q_4, q[a] + 0.01)
            qp_lbs = [q_1, q_2, q_3]
            qp_ubs = [q_2, q_3, q_4]
            ...
            con_1 = @constraint(wm.model, sum(q_par) == wm.var[:nw][n][:qp][a][r_hat])
            ...
            dfp = R_a[r_hat] .* [qp_ubs[k]^1.852 - qp_lbs[k]^1.852 for k in 1:3]
            ...
            qp_constants = [R_a[r_hat] * qp_lbs[k]^1.852 - qp_lbs[k] * qp_slopes[k] for k in 1:3]

            r_rest = setdiff(1:length(R_a), [r_hat])
            qp_ubs = [getupperbound(wm.var[:nw][n][:qp][a][r]) for r in 1:length(R_a)]
            slopes_p = [R_a[r]*qp_ubs[r]^(0.852) for r in 1:length(R_a)]

            term_1 = AffExpr(wm.var[:nw][n][:qp][a][r_rest], slopes_p[r_rest], 0.0)
            term_2 = AffExpr(q_par, qp_slopes, 0.0)
            term_3 = AffExpr(x_par, qp_constants, 0.0)

            con_8 = @constraint(wm.model, dhp / L_a <= term_1 + term_2 + term_3)
        elseif q[a] < 0.0
            dhn = wm.var[:nw][n][:dhn][a]
            q_par = @variable(wm.model, [k in 1:3], category = :Cont, lowerbound = 0.0, start = 0.0)
            x_par = @variable(wm.model, [k in 1:3], category = :Bin, start = 0)
            ...
            # Set up partitioning points.
            q_1 = getlowerbound(wm.var[:nw][n][:qn][a][r_hat])
            q_4 = getupperbound(wm.var[:nw][n][:qn][a][r_hat])
            q_2 = max(q_1, -q[a] - 0.01)
            q_3 = min(q_4, -q[a] + 0.01)
            qn_lbs = [q_1, q_2, q_3]
            qn_ubs = [q_2, q_3, q_4]
            ...
            con_1 = @constraint(wm.model, sum(q_par) == wm.var[:nw][n][:qn][a][r_hat])
            con_2 = @constraint(wm.model, q_par[1] >= x_par[1] * q_1)
            con_3 = @constraint(wm.model, q_par[1] <= x_par[1] * q_2)
            con_4 = @constraint(wm.model, q_par[2] >= x_par[2] * q_2)
            con_5 = @constraint(wm.model, q_par[2] <= x_par[2] * q_3)
            ...
            dfn = R_a[r_hat] .* [qn_ubs[k]^1.852 - qn_lbs[k]^1.852 for k in 1:3]
            ...
            qn_constants = [R_a[r_hat] * qn_lbs[k]^1.852 - qn_lbs[k] * qn_slopes[k] for k in 1:3]

            r_rest = setdiff(1:length(R_a), [r_hat])
            qn_ubs = [getupperbound(wm.var[:nw][n][:qn][a][r]) for r in 1:length(R_a)]
            slopes_n = [R_a[r]*qn_ubs[r]^(0.852) for r in 1:length(R_a)]

            term_1 = AffExpr(wm.var[:nw][n][:qn][a][r_rest], slopes_n[r_rest], 0.0)
            term_2 = AffExpr(q_par, qn_slopes, 0.0)
            term_3 = AffExpr(x_par, qn_constants, 0.0)

            con_8 = @constraint(wm.model, dhn / L_a <= term_1 + term_2 + term_3)
        end
    end
end
