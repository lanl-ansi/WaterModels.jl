...
function _get_bound_problems_nodes(wm::AbstractWaterModel)
    return vcat(_get_bound_problems_nodes.(Ref(wm), nw_ids(wm))...)
end


function _get_bound_problems_nodes(wm::AbstractWaterModel, nw::Int)
    return vcat(_get_bound_problems_node.(Ref(wm), ids(wm, nw, :node), nw)...)
end


function _get_bound_problems_node(wm::AbstractWaterModel, i::Int, nw::Int)
...
function _get_bound_problems_pipes(wm::AbstractWaterModel)
    return vcat(_get_bound_problems_pipes.(Ref(wm), nw_ids(wm))...)
end


function _get_bound_problems_pipes(wm::AbstractWaterModel, nw::Int)
    return vcat(_get_bound_problems_pipe.(Ref(wm), ids(wm, nw, :pipe), nw)...)
end


function _get_bound_problems_pipe(wm::AbstractNCModel, i::Int, nw::Int)
...
function _get_bound_problems_pipe(wm::AbstractNCDModel, i::Int, nw::Int)
    if haskey(var(wm, nw), :q_pipe) && i in [x[1] for x in eachindex(var(wm, nw, :q_pipe))]
        q_vid = _VariableIndex(nw, :pipe, :q_pipe, i)
...
function _get_bound_problems_pumps(wm::AbstractWaterModel)
    return vcat(_get_bound_problems_pumps.(Ref(wm), nw_ids(wm))...)
end


function _get_bound_problems_pumps(wm::AbstractWaterModel, nw::Int)
    return vcat(_get_bound_problems_pump.(Ref(wm), ids(wm, nw, :pump), nw)...)
end


function _get_bound_problems_pump(wm::AbstractWaterModel, i::Int, nw::Int)
...
function _get_bound_problems_regulators(wm::AbstractWaterModel)
    return vcat(_get_bound_problems_regulators.(Ref(wm), nw_ids(wm))...)
end


function _get_bound_problems_regulators(wm::AbstractWaterModel, nw::Int)
    return vcat(_get_bound_problems_regulator.(Ref(wm), ids(wm, nw, :regulator), nw)...)
end


function _get_bound_problems_regulator(wm::AbstractWaterModel, i::Int, nw::Int)
...
function _get_bound_problems_short_pipes(wm::AbstractWaterModel)
    return vcat(_get_bound_problems_short_pipes.(Ref(wm), nw_ids(wm))...)
end


function _get_bound_problems_short_pipes(wm::AbstractWaterModel, nw::Int)
    return vcat(_get_bound_problems_short_pipe.(Ref(wm), ids(wm, nw, :short_pipe), nw)...)
end


function _get_bound_problems_short_pipe(wm::AbstractNCModel, i::Int, nw::Int)
...
function _get_bound_problems_short_pipe(wm::AbstractNCDModel, i::Int, nw::Int)
    if haskey(var(wm, nw), :q_short_pipe) && i in [x[1] for x in eachindex(var(wm, nw, :q_short_pipe))]
        q_vid = _VariableIndex(nw, :short_pipe, :q_short_pipe, i)
...
function _get_bound_problems_valves(wm::AbstractWaterModel)
    return vcat(_get_bound_problems_valves.(Ref(wm), nw_ids(wm))...)
end


function _get_bound_problems_valves(wm::AbstractWaterModel, nw::Int)
    return vcat(_get_bound_problems_valve.(Ref(wm), ids(wm, nw, :valve), nw)...)
end


function _get_bound_problems_valve(wm::AbstractNCModel, i::Int, nw::Int)
...
function _get_bound_problems_valve(wm::AbstractNCDModel, i::Int, nw::Int)
    if haskey(var(wm, nw), :q_valve) && i in [x[1] for x in eachindex(var(wm, nw, :q_valve))]
        q_vid = _VariableIndex(nw, :valve, :q_valve, i)
...
function _get_bound_problems(wm::AbstractWaterModel)::Vector{BoundProblem}
    # Create the sets of bound-tightening problems.
    bps_node = _get_bound_problems_nodes(wm)
    bps_pipe = _get_bound_problems_pipes(wm)
    bps_pump = _get_bound_problems_pumps(wm)
    bps_regulator = _get_bound_problems_regulators(wm)
    bps_short_pipe = _get_bound_problems_short_pipes(wm)
    bps_valve = _get_bound_problems_valves(wm)
